...
class CatHead
{
	...
	public static function run()
	{
		...
		do {
			...
			//随机数校验
			if(!empty($requests['ran']))
			{
				$mcobj = BaseFunction::getMC();
				$strobj = CatConstant::STR_OBJ;
			    if(!$mcobj->setKeep($requests['ran'], $strobj, $timeout=3600))
			    {
			    	$response['code'] = CatConstant::ERROR_MC; $response['desc'] = __LINE__; break;
			    }			   
			}
